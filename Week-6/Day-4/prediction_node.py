...
def prediction_node(state) -> dict:
    ...
    if sub_type == "match":
        ...
        try:
            result = predict_match_winner(team_a, team_b, date=date, team_a_is_home=True)
            explanation = _explain_match_prediction(
                team_a, team_b, True,
    ...
    if sub_type == "player":
        ...
        try:
            ranked = predict_top_player(team, date=date, top_n=top_n)
