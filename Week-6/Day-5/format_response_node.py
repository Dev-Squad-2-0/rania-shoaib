...
Task 1 hardening note: the disclaimer text itself now lives in
hardening.py's DISCLAIMER_* constants instead of being typed out
separately in _format_match_prediction and _format_player_prediction.
Two independent literal copies of safety-critical wording is exactly how
they silently drift apart over time (one edited during a future change,
the other forgotten) -- importing the same constant in both places makes
that impossible by construction.
"""

from hardening import DISCLAIMER_LEAD_IN, DISCLAIMER_MATCH_CLOSE, DISCLAIMER_PLAYER_CLOSE
...
def _format_match_prediction(result: dict) -> str:
    ...
    lines = [
        f"{DISCLAIMER_LEAD_IN} {winner} is favoured to win, "
...
def _format_player_prediction(result: dict) -> str:
    ...
    lines = [f"{DISCLAIMER_LEAD_IN} projected top {len(players)} "
